...
```{r}
flights |> filter(dep_delay > 720) |> count(origin, sort = TRUE)
...
flights |> 
  ...
  summarise(
    avg_airtime_hrs = (mean(air_time, na.rm = TRUE) / 60),
    num_flights     = n()
  )
...
airports |>  count(tzone) |> arrange(desc(n))

airports |> filter(is.na(tzone))

# One airport with a missing time zone is Dillant Hopkins Airport
